...
## The gate adds the comment trim (2026-09-24)

The MUST gate lists the comment trim (`trim-comments`, run by `comment-trimmer`) before the two reviews, with the gate's two outs: an explicit waiver or a superseding repository process. The late trim, as when a session reaches `file-pr` with only the reviews done, sits under the cases that need no new review, in "The review already covers this revision": a trim run after that review keeps its coverage. `trim-comments` states why once (the trim is comment-only, and the session confirms that from its diff), and `file-pr` points there instead of restating it, as does the table row that answers "the reviews passed; the trim is only cosmetic". The trim's encoding offers reach the user before filing. In plan-only probes, a session asked to file after two passing reviews and no trim filed without it 2 of 2 on the 0.42.0 wording ("the comment trim is standard 8 of code-quality-review") and dispatched the trim first 3 of 3 on this wording. Rationale and the full probe record: [trim-comments](trim-comments.md#moved-into-workbench-as-a-delivery-stage-2026-09-24).
