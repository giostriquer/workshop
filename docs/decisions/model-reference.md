...
## The comment-trimmer joins the test-quality exception (2026-09-24)

`comment-trimmer` takes the test-quality review's pins: Opus at `xhigh` on Claude Code through its frontmatter, `gpt-6-sol` at `xhigh` on Codex through its Dispatch line, spawned without the parent's history, and the host's default model elsewhere. The session that wrote the code is blind to its own comment slop, so the trim never inherits the parent's history, and the operator set it at the test reviewer's tier. A first draft gave it a third section that repeated the test-quality one word for word; one section now names both agents, `test-quality-reviewer` and `comment-trimmer`, at that tier, so a later change to the tier cannot split them. The spawn arguments live once, in the agent's Dispatch line. Rationale and probes: [trim-comments](trim-comments.md#moved-into-workbench-as-a-delivery-stage-2026-09-24).
