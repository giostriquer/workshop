...
dedicated ~1,500-word `using-git-worktrees` skill workbench never adopted, using the
...
- No prescribed naming scheme beyond `<task-name>`, no tooling, and no hook.
  This remains informational orientation, consistent with the skill's no-compulsion
  stance.
