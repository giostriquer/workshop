...
2. **Activation doesn't need a second skill.** `handoff-goal`'s emitted
   ...
   contract"). Extending that voice to carry the full pursuit discipline —
...
("on a runtime with durable goal support — e.g. Codex `create_goal` — activate
...
- **Fit check** (first): does the goal fit autonomous pursuit — progress by
...
- **Real-surface verifier + capability inventory**: identify the primary
  ...
  check.
- **Red-team before delivery**: can success be faked by weakening a check?
...
reward-hack regime does not reproduce in bounded single-shot harnesses —
...
`plan.md`, so `goal.md` diffs are anomalous — is verified by inspection of the
emitted sample.
