...
The third handoff the maintainer kept writing by hand sent work *forward* instead of handing a finished branch *backward* to a reviewer (`handoff-review`) or PR opener (`handoff-pr`). Its instruction was, "here's the goal, here's where we are, here are the working rules; new session, go pursue it." Written ad hoc, the document carried the goal but lost the rules. Preferences stated once in chat, such as commit style, push policy, and PR target, entered the file inconsistently. Nothing told the pursuing session what to do when its own context compacted mid-goal.
...
A contract generator, not an executor. It resolves the goal from one of three sources: the session's trajectory (then confirmed), referenced existing work (a plan, slices, or a spec), or a brand-new description (asking only what is needed to make it actionable). A **fit check** follows. Goal handoffs pay off when pursuit is a loop with repeatable attempts, checks that can fail, and no fresh preference decision after each failure; when the shape is wrong, the skill recommends a lighter tool instead. It re-derives current state *and the baseline* from the repo rather than session memory, gathers operating rules (repo rule files + operator statements, asking for what's still open), red-teams the draft, and writes a **split contract**:
...
with:
```markdown
| [`handoff-goal`](handoff-goal.md) | Produces a self-contained goal contract directory for a new session on any runtime (including Codex goal mode) to pursue autonomously across compactions. The directory contains `goal.md`, the frozen contract (outcome, baseline, real-surface primary verifier, integrity rules, approval gates, operating rules), plus `plan.md`, the living route the pursuer maintains. The skill also critiques existing contracts and never pursues the goal itself. |
```
...
```markdown
| `handoff-goal` | a self-contained goal document (goal + definition of done, current state, concrete operating rules) for a new session to pursue autonomously across compactions; the skill never pursues the goal itself |
```
with:
```markdown
| `handoff-goal` | a self-contained goal contract directory for a new session to pursue autonomously across compactions: `goal.md` (frozen contract with outcome, baseline, acceptance checks, a real-surface primary verifier, integrity rules, approval gates, and operating rules) plus `plan.md` (living phases, ledger, and next action). Codex goal mode can activate it directly; the skill never pursues the goal itself. |
```
