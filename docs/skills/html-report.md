...
| --- | --- |
| Findings already exist — on disk or reached in this session — and you want them as a page | `html-report` |
...
candidate counts only if it has an inline `<style>` block and prose content — not
a minified or tool-emitted page. "If one exists, it is the house style." Several
...
governs *visual treatment*; it "never waives the architecture bullets," which
are: single self-contained file, inline CSS and JS; a ~288px sticky TOC sidebar
...
`html-report` ships in **`toolkit`**, the optional plugin — not in the
