...
Keeps an epic auditor lane's final handbacks in one complete, copyable report
...
`workbench:epic-auditor` in audit briefs, follow-ups, amendments and recovery
prompts. Standalone audits, implementation lanes and the epic owner use their
own skills.
...
contract; each final reply still contains all populated fields.
