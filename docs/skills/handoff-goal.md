...
**You invoke it; the session never does.** The skill carries `disable-model-invocation: true`: it runs only when you call `/handoff-goal`, never because a session decided your work looked long-running.
