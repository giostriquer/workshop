...
| Something to check, not yet sized | `audit` — it asks for the tier first |
...
| The discipline | Runtime observation only — never a unit test as a substitute; happy path, **then probe around it** (empty input, conflict, double-submit, a deliberately triggered error) |
