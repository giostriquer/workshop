...
| --- | --- |
| The representation has to be derived by reading code, a diff, or a plan | `arch-map` authors the representation and traces every element to something real |
| Findings already exist on disk or were reached in this session and need rendering | `html-report` renders the findings and may never invent |
...
not an `html-report` report." A sibling that passes the genre test sets tokens and
...
**Not working if** the page is only cards and lists with no diagram because that "fails
