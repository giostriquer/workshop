...
Nobody. Timeouts count as detected, exactly as Stryker and cargo-mutants score them. Uncovered scope is recorded as `partial` with the command under Strategy notes. Neither is an Issue, holds delivery, consumes a follow-up pass, or goes to the caller or an epic owner for a waiver; what the lane ran within its budget is the evidence.
...
It's misapplied if the author's session ran it, missing tools produced a qualitative PASS, a pure move's bodies were mutated, a whole file was swept because one of its tests changed, a follow-up round rebuilt the copy and reran the initial scope, a custom harness was built for unparsed files, the lane ran past its budget, or a mutation decision was sent to the caller or an epic owner.
