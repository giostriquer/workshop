---
name: audit
description: Use when starting from something to verify, hunt, or check — a bug to identify, a refactor to confirm complete, a premise to test ("audit X", "check whether ...", "is this actually done?"). Asks the user to size the workload first, dispatches the right engine, brings flagged uncertainties back for confirmation, then routes the exit by shape. Pure protocol — it never investigates by itself. Not for work that starts from an idea to build (that path grounds against the codebase and goes to brainstorming).
metadata:
  system: workbench
---
...
4. **Confirm the flags — only when there are flags.** Bring each flagged point
   ...
   what reading you lean toward) and wait for their answers. A clean audit —
