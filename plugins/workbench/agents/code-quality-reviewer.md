...
1. Load the `code-quality-review` skill and treat its `SKILL.md` as the **complete** rubric: tone, approval bar, output ordering, and the code-judo / 1k-line / spaghetti rules. It is the single source of truth for how this review is conducted. If the host does not auto-load skills, read the `code-quality-review` skill's SKILL.md bundled with the workbench plugin directly.
