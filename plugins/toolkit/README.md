...
| `trim-comments` | removes comment slop (narration, banners, commented-out code, stale comments, workaround sermons) from the branch diff before review, keeps comments that carry what the code can't, offers encodings for constraint comments, and reports correctness-hiding suppressions; user-invoked only |
