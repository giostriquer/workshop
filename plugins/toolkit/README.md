...
`structure-view` derives an architectural page when no source doc exists — subsystem maps, refactor before/afters, proposed designs — with every box and edge traced to real code.
...
`handoff-goal`, `doc-to-html`, `claim-check`, `qa-sweep`, `empirical-proof`, `code-quality-review`, `get-pr-comments`, `ui-demo-video`, `route-work`, and `structure-view` as skills and bundles the agent files inertly; use the
...
`handoff-pr`, `handoff-goal`, `doc-to-html`, `claim-check`, `qa-sweep`, `empirical-proof`, `code-quality-review`, `get-pr-comments`, `ui-demo-video`, `route-work`, and `structure-view` (skills are invoked by name, not namespaced). The same marketplace also
...
| `structure-view` | a self-contained architectural HTML page derived when no source doc exists — a refactor in flight (before/after or delta view, with the invariant — what does *not* change — stated), an existing subsystem (containment map), or a proposed design (dashed proposed elements) — containment-first layout, role palette with a mandatory legend, provenance stamps, and every box and edge traceable to a real file / symbol / diff hunk; ephemeral-first in `tmp/`, with a promote-to-durable pass on request |
