---
name: handoff-goal
description: Use when a goal — the rest of an in-flight plan, a scoped slice of it, or a newly described but concrete objective — should be handed to a new session to pursue autonomously, or when an existing goal contract needs critique. For already-defined work expected to achieve a concrete result — the outcome stateable up front, done verifiable by checks that can fail. NOT for exploratory handoffs (open-ended research, "look into X" investigations, work whose outcome can't be stated yet) — those fit a plain task or a handoff-review continue brief instead. Produces a self-contained goal contract directory (`tmp/<YYYY-MM-DD>-<goal-slug>/`) holding `goal.md`, the frozen contract (outcome, baseline, acceptance checks with a real-surface primary verifier, integrity rules, approval gates, operating rules), and `plan.md`, the status-tracked route (current state, phases) the pursuing session advances by status flips alone — in progress / done / blocked and checkbox ticks, never prose, evidence, or history, which live in git commits and session output — so pursuit stays consistent across compactions on any runtime, including Codex goal mode, which can activate the contract directly. With no argument it infers the goal from session context and confirms; pointed at an existing goal directory (or "critique") it audits and tightens instead of writing anew; never pursues the goal itself.
---
...
8. **Assemble the contract** from the two templates into `tmp/<YYYY-MM-DD>-<goal-slug>/` (today's date, short kebab-case goal name): `goal.md` and `plan.md`, cross-referencing each other. Two files, nothing else — the contract ships no ledger, log, or evidence file, and pursuit never creates one.
