---
name: test-audit
description: Use when the user asks to gate a test before it lands, to audit existing tests for low-value, implementation-coupled, or duplicative cases and the test-only production seams they keep alive, or to prune one subsystem's whole test surface. Not the completion-gate review of a finished diff's tests. User-invoked only.
...
whole subsystem's test surface (every test file a package, plugin, or core area
...
matches one, and audits and campaigns hunt for existing tests that do.
...
- provider- or plugin-local replays of shared helpers;
...
- core and packages (the main source tree and shared libraries);
...
Keep a test when it independently enforces a public API, SDK or plugin API,
...
Use the repository's own test commands, and route heavy proof the way its
testing docs direct.

1. Run the smallest owner and sibling tests with the repository's focused test
   command (one file or filter, not the full suite).
...
4. Run the changed-files gate the repository's policy requires, after its
   classification or dry-run step when it has one.
...
repository's review and PR or delivery process; when workbench is installed,
its `file-pr` skill files the PR. Land one coherent PR at a time; after
landing, refresh from the current default branch and rerun read-only discovery
