...
such as `plugins/chat`, or one core area. The value bar, retention bar,
...
their own list: they can be real product bugs rather than stale tests (for a
chat plugin, messages that never deliver).
...
prefixes. For a chat plugin such as `plugins/chat` these might be accounts,
commands, context, dispatch, inbound, outbound, persistence, transport, shared,
harness, and live/QA scenarios. Include the subsystem's cases at shared core
...
goes into a written **ledger** with one mark. A parameterized test (`it.each`,
a table of cases) is one declaration unless its rows need different marks; then
mark each row.
...
progress window can assert that the window was _not_ cleared.
...
pass, starting from the ledger, looks for the redundant **layer**. In a chat
plugin, for example, several dispatch suites can replay the same shared
compositor through one mocked preview, around stronger real-stream and
HTTP-fixture suites. Name the **keeper** suite for each contract. Prefer the
real transport boundary with a fake network over a mocked collaborator. Correct
...
Register moved suites in CI routing and test inventories. Update any
shrink-only line-cap baselines. Put durable test-ownership rules in the
subsystem's `AGENTS.md`, drawn from mistakes this campaign actually found.
...
real findings: a full campaign's review can surface several lost contracts and
...
than rebasing a long, many-commit campaign. When the default branch modified a
file the campaign deleted, keep the deletion. Port the new contract into the
keeper instead, and confirm every new regression the default branch added still
has a home. Rerun the whole subsystem suite and repeat live proof on the merged
head.
