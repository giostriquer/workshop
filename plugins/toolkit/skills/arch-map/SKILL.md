...
shapes. Otherwise use the defaults and reference markup below. If this
scaffold has `tmp/architecture-overview.html`, match it as house style.
...
Worked house-style specimen when present in this scaffold:
`tmp/architecture-overview.html` — otherwise match the tokens and scraps
above.
